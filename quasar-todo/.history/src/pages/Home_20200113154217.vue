...
<br>
        <q-btn :loading="loading5" color="green-5" @click="simulateProgress(5)" v-on:click="login()" icon-right="arrow_forward"> Zaloguj się
          <template v-slot:loading>
            <q-spinner-facebook class="on-right"/>
          </template>
        </q-btn>
        <q-btn :loading="loading6" color="blue-5" @click="simulateProgress(6)" v-on:click="passrem()"  > Nie pamiętam hasła
      <template v-slot:loading>
                <q-spinner-facebook class="position-left"/>Sprawdź adres email
      </template>
      </q-btn>
     </div>
