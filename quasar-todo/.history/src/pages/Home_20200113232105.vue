...
<h5 class="q-m-none text-center text-bold" >Witaj na stronie Twojego zdrowszego życia!</h5>

<p class="q-md-none text-center text-body1">Na danej stronie możesz prowadzić swój dziennik postępów w ramach stosowania diety, 
...
<br>
        <q-btn rounded no-caps :loading="loading5" color="green-8" @click="simulateProgress(5)" v-on:click="login()" icon-right="arrow_forward" class="pas">Zaloguj
          <template v-slot:loading>
            <q-spinner-facebook class="on-right"/>
          </template>
        </q-btn>
        <q-btn rounded no-caps :loading="loading6" color="blue-5" @click="simulateProgress(6)" v-on:click="passrem()" width="200px" class="pas">Nie pamiętam hasła
      <template v-slot:loading>
...
<br>
    ...
    <p class="text-body2 text-bold"> Nie masz jeszcze konta? Załóż je! </p>
      <q-btn rounded no-caps color="green" icon="group_add" class="konto"> Stwórz konto
          <template v-slot:loading>
            <q-spinner-facebook class="on-right"/>
          </template>
        </q-btn>
