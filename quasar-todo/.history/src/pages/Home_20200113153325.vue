...
<br>
        <q-btn :loading="loading5" color="green" @click="simulateProgress(5)" v-on:click="login()" icon="arrow_forward"> Zaloguj się
          <template v-slot:loading>
            <q-spinner-facebook class="on-right"/>
          </template>
        </q-btn>
        <q-btn :loading="loading6" color="green" @click="simulateProgress(6)" v-on:click="passrem()" icon="live_help" >Nie pamiętam hasła
      <template v-slot:loading>
                <q-spinner-gears class="on-right"/> Wysyłanie maila z przypomnieniem
      </template>
      </q-btn>
     </div>
