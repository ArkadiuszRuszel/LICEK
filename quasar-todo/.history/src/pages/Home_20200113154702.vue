...
<br>
        <q-btn no-caps :loading="loading5" color="green-5" @click="simulateProgress(5)" v-on:click="login()" icon-right="arrow_forward"> Zaloguj się
          <template v-slot:loading>
            <q-spinner-facebook class="on-right"/>
          </template>
        </q-btn>
        <q-btn no-caps :loading="loading6" color="blue-5" @click="simulateProgress(6)" v-on:click="passrem()" icon-right="arrow_backward"> Nie pamiętam hasła
      <template v-slot:loading>
                <q-spinner-facebook/>Sprawdź adres email
      </template>
      </q-btn>
     </div>
